...
interface NftCardProps {
  ...
  image: string;
  isTwoUSDT: any;
  handleBuyClick: (id: any, name: any, image: any, price: any) => Promise<void>;
}
const BuyNftCard: React.FC<NftCardProps> = ({ id, name, image, isTwoUSDT, handleBuyClick }) => {
  const price: any = parseInt(isTwoUSDT) / 10 ** 18;
      ...
      <div className="relative">
        <div className="flex w-[462px] flex-col gap-5 rounded-[24px] border-[3px] border-solid border-[#5D3068] p-7">
          <div className="flex justify-between">
            <div className="flex items-center gap-[10px]">
              ...
              <div className="nato-fontfamily flex flex-col gap-1">
                ...
                <div className=" text-lg font-bold leading-[21.6px]">{name}</div>
              </div>
            </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <button
              className="nato-fontfamily w-[212px] rounded-[100px] bg-[#AF50BD] px-10 py-5 text-base font-medium uppercase leading-[19.2px]"
              onClick={() => handleBuyClick(id, name, image, price)}
            >
              Buy
            </button>
          </div>
